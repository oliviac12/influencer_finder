    ...
    def __init__(self):
        ...
        if SUPABASE_AVAILABLE:
            try:
                ...
                self.debug_info = {
                    'env_url': 'SET' if supabase_url else 'NOT SET',
                    'env_key': 'SET' if supabase_key else 'NOT SET',
                    'url_length': len(supabase_url) if supabase_url else 0,
                    'key_length': len(supabase_key) if supabase_key else 0,
                    'secrets_available': False,
                    'secrets_url': 'NOT CHECKED',
                    'secrets_key': 'NOT CHECKED'
                }
                    ...
                    try:
                        ...
                        if hasattr(st, 'secrets'):
                            ...
                            if url_in_secrets and key_in_secrets:
                                ...
                                self.debug_info['using_secrets'] = True
                    except Exception as e:
                        self.debug_info['secrets_error'] = str(e)
...
def render_email_outreach_section(app, current_campaign=None):
    ...
    with cache_col2:
        if email_manager.cache_status == "supabase":
            ...
            # Show debug info when credentials are missing
            if hasattr(email_manager, 'debug_info'):
                with st.expander("🔍 Debug: Credential Check"):
                    st.json(email_manager.debug_info)
    ...
    if f"email_drafts_{current_campaign}" not in st.session_state:
        ...
        if missing_drafts == 0 and cached_drafts_available > 0:
            # Auto-load all cached drafts
            ...
            # Show generate button for missing drafts
